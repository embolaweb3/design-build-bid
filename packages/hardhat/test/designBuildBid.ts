...
import { DesignBuildBid } from "../typechain-types";

describe("DesignByBid Contract", function () {
  let contract: DesignBuildBid;
  let owner: any;
  let bidder1: any;
  ...
  before(async function () {
    ...
    const contractFactory = await ethers.getContractFactory("DesignBuildBid");
    contract = (await contractFactory.deploy()) as DesignBuildBid;
    await contract.waitForDeployment();
  });

  it("Should post a project", async function () {
    const tx = await (contract.connect(owner) as Contract).postProject(
      "Project 1",
